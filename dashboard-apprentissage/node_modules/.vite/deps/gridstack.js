//#region node_modules/gridstack/dist/utils.js
/**
* utils.ts 10.3.1
...
function obsoleteOptsDel(opts, oldName, rev, info) {
	if (opts[oldName] !== void 0) console.warn("gridstack.js: Option `" + oldName + "` is deprecated in " + rev + info);
}
...
var Utils = class Utils {
	...
	static getElement(els, root = document) {
		if (typeof els === "string") {
			...
			if (doc && els[0] === "#") return doc.getElementById(els.substring(1));
			if (els[0] === "#" || els[0] === "." || els[0] === "[") return root.querySelector(els);
			if (doc && !isNaN(+els[0])) return doc.getElementById(els);
	...
	*/
	...
	static removeStylesheet(id, parent) {
		let el = (parent || document).querySelector("STYLE[gs-style-id=" + id + "]");
		if (el && el.parentNode) el.remove();
	}
	/** inserts a CSS rule */
	static addCSSRule(sheet, selector, rules) {
		if (typeof sheet.addRule === "function") sheet.addRule(selector, rules);
		else if (typeof sheet.insertRule === "function") sheet.insertRule(`${selector}{${rules}}`);
	}
	static toBool(v) {
	...
	static parseHeight(val) {
		let h;
		let unit = "px";
		if (typeof val === "string") if (val === "auto" || val === "") h = 0;
		else {
			let match = val.match(/^(-[0-9]+\.[0-9]+|[0-9]*\.[0-9]+|-[0-9]+|[0-9]+)(px|em|rem|vh|vw|%|cm|mm)?$/);
			if (!match) throw new Error(`Invalid height val = ${val}`);
			unit = match[2] || "px";
	...
	static defaults(target, ...sources) {
		sources.forEach((source) => {
			for (const key in source) {
				...
				if (target[key] === null || target[key] === void 0) target[key] = source[key];
				else if (typeof source[key] === "object" && typeof target[key] === "object") this.defaults(target[key], source[key]);
			}
		});
		return target;
	}
	...
	static copyPos(a, b, doMinMax = false) {
		if (b.x !== void 0) a.x = b.x;
		if (b.y !== void 0) a.y = b.y;
		if (b.w !== void 0) a.w = b.w;
		if (b.h !== void 0) a.h = b.h;
		if (doMinMax) {
	...
	static removeInternalAndSame(a, b) {
		...
		for (let key in a) {
			const aVal = a[key];
			const bVal = b[key];
			if (key[0] === "_" || aVal === bVal) delete a[key];
	...
	static removeInternalForSave(n, removeEl = true) {
		for (let key in n) if (key[0] === "_" || n[key] === null || n[key] === void 0) delete n[key];
		delete n.grid;
	...
	static removePositioningStyles(el) {
		...
		if (style.height) style.removeProperty("height");
	}
	...
	static getScrollElement(el) {
		...
		else return this.getScrollElement(el.parentElement);
	}
	/** @internal */
	static updateScrollPosition(el, position, distance) {
		...
		if (rect.top < 0 || rect.bottom > innerHeightOrClientHeight) {
			...
			if (scrollEl !== null) {
				...
				if (rect.top < 0 && distance < 0) if (el.offsetHeight > innerHeightOrClientHeight) scrollEl.scrollTop += distance;
				else scrollEl.scrollTop += Math.abs(offsetDiffUp) > Math.abs(distance) ? distance : offsetDiffUp;
				else if (distance > 0) if (el.offsetHeight > innerHeightOrClientHeight) scrollEl.scrollTop += distance;
				else scrollEl.scrollTop += offsetDiffDown > distance ? distance : offsetDiffDown;
	...
	*/
	static updateScrollResize(event, el, distance) {
		...
		if (top) scrollEl.scrollBy({
			behavior: "smooth",
			top: pointerPosY - distance
		});
		else if (bottom) scrollEl.scrollBy({
			behavior: "smooth",
	...
	*/
	static cloneDeep(obj) {
		...
		for (const key in ret) if (ret.hasOwnProperty(key) && typeof ret[key] === "object" && key.substring(0, 2) !== "__" && !skipFields.find((k) => k === key)) ret[key] = Utils.cloneDeep(obj[key]);
		return ret;
	}
	...
	static appendTo(el, parent) {
		let parentNode;
		if (typeof parent === "string") parentNode = Utils.getElement(parent);
		else parentNode = parent;
		if (parentNode) parentNode.appendChild(el);
	}
	static addElStyles(el, styles) {
		if (styles instanceof Object) {
			for (const s in styles) if (styles.hasOwnProperty(s)) if (Array.isArray(styles[s])) styles[s].forEach((val) => {
				el.style[s] = val;
			});
			else el.style[s] = styles[s];
		}
	}
	...
	static simulateMouseEvent(e, simulatedType, target) {
		...
		simulatedEvent.initMouseEvent(simulatedType, true, true, window, 1, e.screenX, e.screenY, e.clientX, e.clientY, e.ctrlKey, e.altKey, e.shiftKey, e.metaKey, 0, e.target);
...
//#endregion
//#region node_modules/gridstack/dist/gridstack-engine.js
/**
...
*/
var GridStackEngine = class GridStackEngine {
	constructor(opts = {}) {
		this.addedNodes = [];
		this.removedNodes = [];
	...
	* return true if we moved. */
	_fixCollisions(node, nn = node, collide, opt = {}) {
		...
		while (collide = collide || this.collide(node, area, opt.skip)) {
			let moved;
			if (collide.locked || this._loading || node._moving && !node._skipDown && nn.y > node.y && !this.float && (!this.collide(collide, {
				...collide,
				y: node.y
				...
				if ((collide.locked || this._loading) && moved) Utils.copyPos(nn, node);
			...
			} else moved = this.moveNode(collide, {
				...collide,
				y: nn.y + nn.h,
				skip: node,
				...newOpt
	...
	directionCollideCoverage(node, o, collides) {
		...
		for (let n of collides) {
			if (n.locked || !n._rect) break;
			let r2 = n._rect;
			let yOver = Number.MAX_VALUE, xOver = Number.MAX_VALUE;
			if (r0.y < r2.y) yOver = (r.y + r.h - r2.y) / r2.h;
			else if (r0.y + r0.h > r2.y + r2.h) yOver = (r2.y + r2.h - r.y) / r2.h;
			if (r0.x < r2.x) xOver = (r.x + r.w - r2.x) / r2.w;
			else if (r0.x + r0.w > r2.x + r2.w) xOver = (r2.x + r2.w - r.x) / r2.w;
	...
	compact(layout = "compact", doSort = true) {
		...
		if (!wasColumnResize) this._inColumnResize = true;
	...
	set float(val) {
		...
		if (!val) this._packNodes()._notify();
	}
	/** float getter method */
	get float() {
	...
	_packNodes() {
		...
		this.sortNodes();
		if (this.float) this.nodes.forEach((n) => {
			if (n._updating || n._orig === void 0 || n.y === n._orig.y) return;
			let newY = n.y;
		...
		else this.nodes.forEach((n, i) => {
			if (n.locked) return;
			while (n.y > 0) {
				let newY = i === 0 ? 0 : n.y - 1;
				if (!(i === 0 || !this.collide(n, {
					x: n.x,
					y: newY,
					w: n.w,
					h: n.h
	...
	*/
	prepareNode(node, resizing) {
		...
		if (node.x === void 0 || node.y === void 0 || node.x === null || node.y === null) node.autoPosition = true;
		let defaults = {
			x: 0,
			y: 0,
			w: 1,
	...
	nodeBoundFix(node, resizing) {
		...
		if ((node.x || 0) + (node.w || 1) > this.column && this.column < 12 && !this._inColumnResize && node._id && this.findCacheLayout(node, 12) === -1) {
			let copy = { ...node };
		...
		else if (node.w < 1) node.w = 1;
		if (this.maxRow && node.h > this.maxRow) node.h = this.maxRow;
		else if (node.h < 1) node.h = 1;
		if (node.x < 0) node.x = 0;
		if (node.y < 0) node.y = 0;
		if (node.x + node.w > this.column) if (resizing) node.w = this.column - node.x;
		else node.x = this.column - node.w;
		if (this.maxRow && node.y + node.h > this.maxRow) if (resizing) node.h = this.maxRow - node.y;
		else node.y = this.maxRow - node.h;
		if (!Utils.samePos(node, before)) node._dirty = true;
		return this;
	}
	/** returns a list of modified nodes from their original values */
	getDirtyNodes(verify) {
		if (verify) return this.nodes.filter((n) => n._dirty && !Utils.samePos(n, n._orig));
	...
	*/
	...
	removeNode(node, removeDOM = true, triggerEvent = false) {
		if (!this.nodes.find((n) => n._id === node._id)) return this;
		if (triggerEvent) this.removedNodes.push(node);
		if (removeDOM) node._removeDOM = true;
		this.nodes = this.nodes.filter((n) => n._id !== node._id);
		if (!node._isAboutToRemove) this._packNodes();
		this._notify([node]);
		return this;
	}
	...
	* others in a clone first, then apply those changes if still within specs. */
	moveNodeCheck(node, o) {
		...
		o.pack = true;
		if (!this.maxRow) return this.moveNode(node, o);
		let clonedNode;
	...
	moveNode(node, o) {
		...
		if (o.pack === void 0 && !this.batchMode) wasUndefinedPack = o.pack = true;
		...
		if (collides.length) {
			...
			if (activeDrag && collide && node.grid?.opts?.subGridDynamic && !node.grid._isTemp) {
				...
				if (over / (a1 < a2 ? a1 : a2) > .8) {
					collide.grid.makeSubGrid(collide.el, void 0, node);
					collide = void 0;
				}
			}
			if (collide) needToMove = !this._fixCollisions(node, nn, collide, o);
			else {
				needToMove = false;
		...
		if (o.pack) this._packNodes()._notify();
	...
	* returning a list of widgets for serialization */
	...
	layoutsNodesChange(nodes) {
		...
		this._layouts.forEach((layout, column) => {
			...
			if (column < this.column) this._layouts[column] = void 0;
			else {
				let ratio = column / this.column;
				nodes.forEach((node) => {
					if (!node._orig) return;
					let n = layout.find((l) => l._id === node._id);
					if (!n) return;
					if (n.y >= 0 && node.y !== node._orig.y) n.y += node.y - node._orig.y;
					if (node.x !== node._orig.x) n.x = Math.round(node.x * ratio);
					if (node.w !== node._orig.w) n.w = Math.round(node.w * ratio);
				});
			}
		});
		return this;
	...
	*/
	columnChanged(prevColumn, column, layout = "moveScale") {
		...
		if (column > prevColumn && this._layouts) {
			...
			if (!cacheNodes.length && prevColumn !== lastIndex && this._layouts[lastIndex]?.length) {
				...
				this._layouts[lastIndex].forEach((cacheNode) => {
					...
					if (n) {
						...
						n.w = cacheNode.w ?? n.w;
						if (cacheNode.x == void 0 || cacheNode.y === void 0) n.autoPosition = true;
					}
				});
			}
			cacheNodes.forEach((cacheNode) => {
				...
				if (j !== -1) {
					...
					if (cacheNode.autoPosition || isNaN(cacheNode.x) || isNaN(cacheNode.y)) this.findEmptyPosition(cacheNode, newNodes);
		...
		if (doCompact) this.compact(layout, false);
		else {
			if (nodes.length) if (typeof layout === "function") layout(column, prevColumn, newNodes, nodes);
			else {
	...
	*/
	cacheLayout(nodes, column, clear = false) {
		let copy = [];
		nodes.forEach((n, i) => {
			if (n._id === void 0) n._id = (n.id ? this.nodes.find((n2) => n2.id === n.id) : void 0)?._id ?? GridStackEngine._idSeq++;
			copy[i] = {
				x: n.x,
				y: n.y,
				w: n.w,
	...
	*/
	...
	removeNodeFromLayoutCache(n) {
		...
		for (let i = 0; i < this._layouts.length; i++) {
			...
			if (index !== -1) this._layouts[i].splice(index, 1);
		}
	}
	/** called to remove all internal values but the _id */
	cleanupNode(node) {
		for (let prop in node) if (prop[0] === "_" && prop !== "_id") delete node[prop];
		return this;
	}
};
...
//#endregion
//#region node_modules/gridstack/dist/dd-manager.js
/**
...
var DDManager = class {};
//#endregion
//#region node_modules/gridstack/dist/dd-touch.js
/**
* touch.ts 10.3.1
* Copyright (c) 2021 Alain Dumesny - see GridStack root license
*/
...
function simulatePointerMouseEvent(e, simulatedType) {
	if (e.cancelable) e.preventDefault();
	const simulatedEvent = document.createEvent("MouseEvents");
	simulatedEvent.initMouseEvent(simulatedType, true, true, window, 1, e.screenX, e.screenY, e.clientX, e.clientY, false, false, false, false, 0, null);
	e.target.dispatchEvent(simulatedEvent);
}
/**
...
function touchend(e) {
	...
	if (!wasDragging) simulateMouseEvent(e, "click");
...
* Copyright (c) 2021-2022 Alain Dumesny - see GridStack root license
*/
var DDResizableHandle = class DDResizableHandle {
	...
	_mouseMove(e) {
		...
		if (this.moving) this._triggerEvent("move", e);
...
//#endregion
//#region node_modules/gridstack/dist/dd-resizable.js
/**
* dd-resizable.ts 10.3.1
* Copyright (c) 2021-2022 Alain Dumesny - see GridStack root license
*/
var DDResizable = class DDResizable extends DDBaseImplement {
	...
	updateOption(opts) {
		...
		if (updateAutoHide) this._setupAutoHide(this.option.autoHide);
		return this;
	}
	...
	_setupAutoHide(auto) {
		if (auto) {
			...
			if (DDManager.overResizeElement === this) delete DDManager.overResizeElement;
		}
		return this;
	}
	/** @internal */
	...
	_resizeStart(event) {
		...
		if (this.option.start) this.option.start(ev, this._ui());
	...
	_resizing(event, dir) {
		...
		if (this.option.resize) this.option.resize(ev, this._ui());
	...
	_resizeStop(event) {
		...
		if (this.option.stop) this.option.stop(ev);
	...
	_setupHelper() {
		...
		if (getComputedStyle(this.el.parentElement).position.match(/static/)) this.el.parentElement.style.position = "relative";
	...
	_getChange(event, dir) {
		...
		let moveLeft;
		let moveUp;
		if (dir.indexOf("e") > -1) newRect.width += offsetX;
		else if (dir.indexOf("w") > -1) {
			newRect.width -= offsetX;
			newRect.left += offsetX;
			moveLeft = true;
		}
		if (dir.indexOf("s") > -1) newRect.height += offsetY;
		...
		if (Math.round(newRect.width) !== Math.round(constrain.width)) {
			if (dir.indexOf("w") > -1) newRect.left += newRect.width - constrain.width;
			newRect.width = constrain.width;
		}
		if (Math.round(newRect.height) !== Math.round(constrain.height)) {
			if (dir.indexOf("n") > -1) newRect.top += newRect.height - constrain.height;
	...
	_constrainSize(oWidth, oHeight, moveLeft, moveUp) {
		...
		return {
			width: Math.min(maxWidth, Math.max(minWidth, oWidth)),
			height: Math.min(maxHeight, Math.max(minHeight, oHeight))
		};
	}
	/** @internal */
	_applyChange() {
		...
		if (this.el.style.position === "absolute") {
			const { left, top } = this.el.parentElement.getBoundingClientRect();
			containmentRect = {
				left,
				top,
				width: 0,
...
var DDDraggable = class DDDraggable extends DDBaseImplement {
	constructor(el, option = {}) {
		super();
		this.el = el;
		this.option = option;
	...
	_callDrag(e) {
		...
		if (this.option.drag) this.option.drag(ev, this.ui());
	...
	_mouseMove(e) {
		...
		if (this.dragging) {
			...
			*/
			...
			if (grid) DDManager.dropElement = grid.el.ddElement.ddDroppable;
			else delete DDManager.dropElement;
			...
			if (this.option.start) this.option.start(ev, this.ui());
	...
	_mouseUp(e) {
		...
		if (this.dragging) {
			...
			document.removeEventListener("keydown", this._keyEvent);
			if (DDManager.dropElement?.el === this.el.parentElement) delete DDManager.dropElement;
			this.helperContainment.style.position = this.parentOriginStylePosition || null;
			if (this.helper === this.el) this._removeHelperStyle();
			...
			if (DDManager.dropElement) DDManager.dropElement.drop(e);
		}
		delete this.helper;
		delete this.mouseDownEvent;
		delete DDManager.dragElement;
	...
	_createHelper(event) {
		let helper = this.el;
		if (typeof this.option.helper === "function") helper = this.option.helper(event);
		else if (this.option.helper === "clone") helper = Utils.cloneNode(this.el);
		if (!document.body.contains(helper)) Utils.appendTo(helper, this.option.appendTo === "parent" ? this.el.parentElement : this.option.appendTo);
		if (helper === this.el) this.dragElementOriginStyle = DDDraggable.originStyleProp.map((prop) => this.el.style[prop]);
		return helper;
	}
	...
	_setupHelperStyle(e) {
		...
		setTimeout(() => {
			if (this.helper) style.transition = null;
		}, 0);
		return this;
	}
	...
	_removeHelperStyle() {
		...
		if (!(this.helper?.gridstackNode)?._isAboutToRemove && this.dragElementOriginStyle) {
	...
	_setupHelperContainmentStyle() {
		...
		if (this.helper.style.position !== "fixed") {
			...
			if (getComputedStyle(this.helperContainment).position.match(/static/)) this.helperContainment.style.position = "relative";
		}
		return this;
	}
	/** @internal */
...
var DDDroppable = class extends DDBaseImplement {
	...
	_mouseEnter(e) {
		...
		e.preventDefault();
		e.stopPropagation();
		if (DDManager.dropElement && DDManager.dropElement !== this) DDManager.dropElement._mouseLeave(e, true);
		DDManager.dropElement = this;
		const ev = Utils.initEvent(e, {
			target: this.el,
			type: "dropover"
		});
		if (this.option.over) this.option.over(ev, this._ui(DDManager.dragElement));
	...
	_mouseLeave(e, calledByEnter = false) {
		...
		if (this.option.out) this.option.out(ev, this._ui(DDManager.dragElement));
		...
		if (DDManager.dropElement === this) {
			...
			if (!calledByEnter) {
				...
				if (parentDrop) parentDrop._mouseEnter(e);
			}
		}
	}
	...
	drop(e) {
		...
		if (this.option.drop) this.option.drop(ev, this._ui(DDManager.dragElement));
	...
	_setupAccept() {
		...
		if (typeof this.option.accept === "string") this.accept = (el) => el.classList.contains(this.option.accept) || el.matches(this.option.accept);
		else this.accept = this.option.accept;
		return this;
	}
	/** @internal */
	_ui(drag) {
...
//#endregion
//#region node_modules/gridstack/dist/dd-element.js
/**
* dd-elements.ts 10.3.1
* Copyright (c) 2021 Alain Dumesny - see GridStack root license
*/
var DDElement = class DDElement {
	static init(el) {
	...
	on(eventName, callback) {
		if (this.ddDraggable && [
			...
			"dragstart",
			"dragstop"
		].indexOf(eventName) > -1) this.ddDraggable.on(eventName, callback);
		else if (this.ddDroppable && [
			"drop",
			"dropover",
			"dropout"
		].indexOf(eventName) > -1) this.ddDroppable.on(eventName, callback);
		else if (this.ddResizable && [
			"resizestart",
			"resize",
			"resizestop"
		].indexOf(eventName) > -1) this.ddResizable.on(eventName, callback);
		return this;
	}
	off(eventName) {
		if (this.ddDraggable && [
			...
			"dragstart",
			"dragstop"
		].indexOf(eventName) > -1) this.ddDraggable.off(eventName);
		else if (this.ddDroppable && [
			"drop",
			"dropover",
			"dropout"
		].indexOf(eventName) > -1) this.ddDroppable.off(eventName);
		else if (this.ddResizable && [
			"resizestart",
			"resize",
			"resizestop"
		].indexOf(eventName) > -1) this.ddResizable.off(eventName);
		return this;
	}
	setupDraggable(opts) {
		if (!this.ddDraggable) this.ddDraggable = new DDDraggable(this.el, opts);
		else this.ddDraggable.updateOption(opts);
		return this;
	}
	cleanDraggable() {
		if (this.ddDraggable) {
	...
	setupResizable(opts) {
		if (!this.ddResizable) this.ddResizable = new DDResizable(this.el, opts);
		else this.ddResizable.updateOption(opts);
		return this;
	}
	cleanResizable() {
		if (this.ddResizable) {
	...
	setupDroppable(opts) {
		if (!this.ddDroppable) this.ddDroppable = new DDDroppable(this.el, opts);
		else this.ddDroppable.updateOption(opts);
		return this;
	}
	cleanDroppable() {
		if (this.ddDroppable) {
...
//#endregion
//#region node_modules/gridstack/dist/dd-gridstack.js
/**
...
var DDGridStack = class {
	resizable(el, opts, key, value) {
		this._getDDElements(el).forEach((dEl) => {
			if (opts === "disable" || opts === "enable") dEl.ddResizable && dEl.ddResizable[opts]();
			else if (opts === "destroy") dEl.ddResizable && dEl.cleanResizable();
			else if (opts === "option") dEl.setupResizable({ [key]: value });
			else {
	...
	draggable(el, opts, key, value) {
		this._getDDElements(el).forEach((dEl) => {
			if (opts === "disable" || opts === "enable") dEl.ddDraggable && dEl.ddDraggable[opts]();
			else if (opts === "destroy") dEl.ddDraggable && dEl.cleanDraggable();
			else if (opts === "option") dEl.setupDraggable({ [key]: value });
			else {
				const grid = dEl.el.gridstackNode.grid;
				dEl.setupDraggable({
					...grid.opts.draggable,
	...
	droppable(el, opts, key, value) {
		...
		this._getDDElements(el).forEach((dEl) => {
			if (opts === "disable" || opts === "enable") dEl.ddDroppable && dEl.ddDroppable[opts]();
			else if (opts === "destroy") {
				if (dEl.ddDroppable) dEl.cleanDroppable();
			} else if (opts === "option") dEl.setupDroppable({ [key]: value });
			else dEl.setupDroppable(opts);
		});
		return this;
	}
	...
	_getDDElements(els, create = true) {
		...
		if (!create) list.filter((d) => d);
		return list;
	}
};
//#endregion
//#region node_modules/gridstack/dist/gridstack.js
/*!
* GridStack 10.3.1
...
var GridStack = class GridStack {
	...
	*/
	static init(options = {}, elOrString = ".grid-stack") {
		...
		if (!el) {
			if (typeof elOrString === "string") console.error("GridStack.initAll() no grid was found with selector \"" + elOrString + "\" - element missing or wrong selector ?\nNote: \".grid-stack\" is required for proper CSS styling and drag/drop, and is the default selector.");
			else console.error("GridStack.init() no grid element was passed.");
			return null;
		}
		if (!el.gridstack) el.gridstack = new GridStack(el, Utils.cloneDeep(options));
		return el.gridstack;
	}
	/**
	...
	*/
	static initAll(options = {}, selector = ".grid-stack") {
		let grids = [];
		if (typeof document === "undefined") return grids;
		GridStack.getGridElements(selector).forEach((el) => {
			if (!el.gridstack) el.gridstack = new GridStack(el, Utils.cloneDeep(options));
			grids.push(el.gridstack);
		});
		if (grids.length === 0) console.error("GridStack.initAll() no grid was found with selector \"" + selector + "\" - element missing or wrong selector ?\nNote: \".grid-stack\" is required for proper CSS styling and drag/drop, and is the default selector.");
		return grids;
	}
	/**
	...
	*/
	static addGrid(parent, opt = {}) {
		...
		if (el.gridstack) {
			...
			if (opt.children !== void 0) grid.load(opt.children);
			return grid;
		}
		if (!parent.classList.contains("grid-stack") || GridStack.addRemoveCB) if (GridStack.addRemoveCB) el = GridStack.addRemoveCB(parent, opt, true, true);
		else {
	...
	*/
	...
	get placeholder() {
		if (!this._placeholder) {
			...
			if (this.opts.placeholderText) placeholderChild.innerHTML = this.opts.placeholderText;
	...
	*/
	constructor(el, opts = {}) {
		...
		el.gridstack = this;
		opts = opts || {};
		if (!el.classList.contains("grid-stack")) this.el.classList.add("grid-stack");
		if (opts.row) {
			opts.minRow = opts.maxRow = opts.row;
			delete opts.row;
		}
		let rowAttr = Utils.toNumber(el.getAttribute("gs-row"));
		if (opts.column === "auto") delete opts.column;
		if (opts.alwaysShowResizeHandle !== void 0) opts._alwaysShowResizeHandle = opts.alwaysShowResizeHandle;
		...
		if (resp) if (!resp.columnWidth && !resp.breakpoints?.length) {
			delete opts.columnOpts;
			bk = void 0;
		} else resp.columnMax = resp.columnMax || 12;
		...
		let defaults = {
			...
			draggable: { handle: (opts.handleClass ? "." + opts.handleClass : opts.handle ? opts.handle : "") || gridDefaults.draggable.handle },
			removableOptions: {
				accept: opts.itemClass || gridDefaults.removableOptions.accept,
				decline: gridDefaults.removableOptions.decline
			}
		};
		if (el.getAttribute("gs-animate")) defaults.animate = Utils.toBool(el.getAttribute("gs-animate"));
		opts = Utils.defaults(opts, defaults);
		this._initMargin();
		this.checkDynamicColumn();
		this.el.classList.add("gs-" + opts.column);
		if (opts.rtl === "auto") opts.rtl = el.style.direction === "rtl";
		if (opts.rtl) this.el.classList.add("grid-stack-rtl");
		const grandParent = this.el.parentElement?.parentElement;
		let parentGridItem = grandParent?.classList.contains(gridDefaults.itemClass) ? grandParent.gridstackNode : void 0;
		if (parentGridItem) {
		...
		if (this._isAutoCellHeight || opts.cellHeight === "initial") this.cellHeight(void 0, false);
		else {
		...
		if (opts.alwaysShowResizeHandle === "mobile") opts.alwaysShowResizeHandle = isTouch;
		...
		if (opts.children) {
			...
			if (children.length) this.load(children);
		}
		this.setAnimation();
		if (opts.subGridDynamic && !DDManager.pauseDrag) DDManager.pauseDrag = true;
		if (opts.draggable?.pause !== void 0) DDManager.pauseDrag = opts.draggable.pause;
	...
	*/
	addWidget(els, options) {
		...
		if (typeof els === "string") {
			...
			node = options = els;
			if (node?.el) el = node.el;
			else if (GridStack.addRemoveCB) el = GridStack.addRemoveCB(this.el, options, true, false);
			else {
	...
	*/
	makeSubGrid(el, ops, nodeToAdd, saveContent = true) {
		...
		if (!node) node = this.makeWidget(el).gridstackNode;
		if (node.subGrid?.el) return node.subGrid;
		let subGridTemplate;
		let grid = this;
		...
		if (saveContent) {
			...
			if (GridStack.addRemoveCB) newItem = GridStack.addRemoveCB(this.el, newItemOpt, true, false);
			else {
				let doc = document.implementation.createHTMLDocument("");
		...
		if (nodeToAdd?._moving) subGrid._isTemp = true;
		if (autoColumn) subGrid._autoColumn = true;
		if (saveContent) subGrid.addWidget(newItem, newItemOpt);
		if (nodeToAdd) if (nodeToAdd._moving) window.setTimeout(() => Utils.simulateMouseEvent(nodeToAdd._event, "mouseenter", subGrid.el), 0);
		else subGrid.addWidget(node.el, node);
		return subGrid;
	}
	/**
	...
	*/
	save(saveContent = true, saveGridOpt = false, saveCB = GridStack.saveCB) {
		...
		if (saveGridOpt) {
			...
			if (this._autoColumn) o.column = "auto";
			const origShow = o._alwaysShowResizeHandle;
			delete o._alwaysShowResizeHandle;
			if (origShow !== void 0) o.alwaysShowResizeHandle = origShow;
			else delete o.alwaysShowResizeHandle;
	...
	*/
	load(items, addRemove = GridStack.addRemoveCB || true) {
		...
		if (!blank && addRemove) [...this.engine.nodes].forEach((n) => {
			if (!n.id) return;
		...
		items.forEach((w) => {
			...
			if (item) {
				...
				if (Utils.samePos(item, w)) this.moveNode(item, {
					...w,
					forceCollide: true
				});
				...
				if (w.subGridOpts?.children) {
					...
					if (sub && sub.gridstack) sub.gridstack.load(w.subGridOpts.children);
				}
			} else if (addRemove) this.addWidget(w);
		});
		delete this.engine._loading;
	...
	*/
	getCellHeight(forcePixel = false) {
		if (this.opts.cellHeight && this.opts.cellHeight !== "auto" && (!forcePixel || !this.opts.cellHeightUnit || this.opts.cellHeightUnit === "px")) return this.opts.cellHeight;
	...
	*/
	cellHeight(val, update = true) {
		...
		if (val === "initial" || val === "auto") val = void 0;
		if (val === void 0) {
			let marginDiff = -this.opts.marginRight - this.opts.marginLeft + this.opts.marginTop + this.opts.marginBottom;
			val = this.cellWidth() + marginDiff;
		}
		let data = Utils.parseHeight(val);
		if (this.opts.cellHeightUnit === data.unit && this.opts.cellHeight === data.h) return this;
		this.opts.cellHeightUnit = data.unit;
		this.opts.cellHeight = data.h;
		this.resizeToContentCheck();
		if (update) this._updateStyles(true);
		return this;
	}
	...
	checkDynamicColumn() {
		...
		if (resp.columnWidth) newColumn = Math.min(Math.round(w / resp.columnWidth) || 1, resp.columnMax);
		else {
			newColumn = resp.columnMax;
			let i = 0;
			while (i < resp.breakpoints.length && w <= resp.breakpoints[i].w) newColumn = resp.breakpoints[i++].c || column;
		}
		if (newColumn !== column) {
	...
	*/
	getCellFromPixel(position, useDocRelative = false) {
		...
		let containerPos;
		if (useDocRelative) containerPos = {
	...
	*/
	makeWidget(els, options) {
		...
		this._updateContainerHeight();
		if (node.subGridOpts) this.makeSubGrid(el, node.subGridOpts, void 0, false);
		if (this.opts.column === 1) this._ignoreLayoutsNodeChange = true;
		this._triggerAddEvent();
		this._triggerChangeEvent();
		delete this._ignoreLayoutsNodeChange;
		return el;
	...
	on(name, callback) {
		if (name.indexOf(" ") !== -1) {
			name.split(" ").forEach((name) => this.on(name, callback));
			return this;
		}
		if (name === "change" || name === "added" || name === "removed" || name === "enable" || name === "disable") {
			if (name === "enable" || name === "disable") this._gsEventHandler[name] = (event) => callback(event);
			else this._gsEventHandler[name] = (event) => callback(event, event.detail);
			this.el.addEventListener(name, this._gsEventHandler[name]);
		} else if (name === "drag" || name === "dragstart" || name === "dragstop" || name === "resizestart" || name === "resize" || name === "resizestop" || name === "dropped" || name === "resizecontent") this._gsEventHandler[name] = callback;
		else console.error("GridStack.on(" + name + ") event not supported");
		return this;
	}
	/**
	...
	*/
	off(name) {
		if (name.indexOf(" ") !== -1) {
			name.split(" ").forEach((name) => this.off(name));
			return this;
		}
		if (name === "change" || name === "added" || name === "removed" || name === "enable" || name === "disable") {
			if (this._gsEventHandler[name]) this.el.removeEventListener(name, this._gsEventHandler[name]);
		}
	...
	*/
	removeWidget(els, removeDOM = true, triggerEvent = true) {
		GridStack.getElements(els).forEach((el) => {
			if (el.parentElement && el.parentElement !== this.el) return;
			let node = el.gridstackNode;
			if (!node) node = this.engine.nodes.find((n) => el === n.el);
			if (!node) return;
			if (removeDOM && GridStack.addRemoveCB) GridStack.addRemoveCB(this.el, node, false, false);
			delete el.gridstackNode;
			this._removeDD(el);
			this.engine.removeNode(node, removeDOM, triggerEvent);
			if (removeDOM && el.parentElement) el.remove();
		});
		if (triggerEvent) {
			this._triggerRemoveEvent();
			this._triggerChangeEvent();
	...
	*/
	removeAll(removeDOM = true, triggerEvent = true) {
		this.engine.nodes.forEach((n) => {
			if (removeDOM && GridStack.addRemoveCB) GridStack.addRemoveCB(this.el, n, false, false);
	...
	*/
	setAnimation(doAnimate = this.opts.animate, delay) {
		...
		else if (doAnimate) this.el.classList.add("grid-stack-animate");
		else this.el.classList.remove("grid-stack-animate");
		return this;
	}
	/** @internal */
	hasAnimationCSS() {
	...
	*/
	update(els, opt) {
		...
		GridStack.getElements(els).forEach((el) => {
			...
			if (!m && (w.minW || w.minH || w.maxW || w.maxH)) m = {};
			if (w.content !== void 0) {
				...
				if (itemContent && itemContent.innerHTML !== w.content) {
					...
					if (n.subGrid?.el) {
						...
						if (!n.subGrid.opts.styleInHead) n.subGrid._updateStyles(true);
					}
				}
				delete w.content;
			}
			let changed = false;
			let ddChanged = false;
			for (const key in w) if (key[0] !== "_" && n[key] !== w[key]) {
				n[key] = w[key];
				changed = true;
			...
			if (m || changed) this._writeAttr(el, n);
			if (ddChanged) this._prepareDragDropByNode(n);
		});
		return this;
	}
	moveNode(n, m) {
	...
	*/
	resizeToContent(el) {
		...
		if (!n) return;
		const grid = n.grid;
		if (!grid || el.parentElement !== grid.el) return;
		...
		let wantedH;
		if (n.subGrid) wantedH = n.subGrid.getRow() * n.subGrid.getCellHeight(true);
		else if (n.subGridOpts?.children?.length) return;
		else {
	...
	*/
	...
	_triggerChangeEvent() {
		...
		if (elements && elements.length) {
			if (!this._ignoreLayoutsNodeChange) this.engine.layoutsNodesChange(elements);
	...
	_triggerAddEvent() {
		...
		if (this.engine.addedNodes?.length) {
			if (!this._ignoreLayoutsNodeChange) this.engine.layoutsNodesChange(this.engine.addedNodes);
	...
	_updateStyles(forceUpdate = false, maxH) {
		if (forceUpdate) this._removeStylesheet();
		if (maxH === void 0) maxH = this.getRow();
		this._updateContainerHeight();
		if (this.opts.cellHeight === 0) return this;
		let cellHeight = this.opts.cellHeight;
		let cellHeightUnit = this.opts.cellHeightUnit;
		let prefix = `.${this._styleSheetClass} > .${this.opts.itemClass}`;
		if (!this._styles) {
			const styleLocation = this.opts.styleInHead ? void 0 : this.el.parentNode;
			this._styles = Utils.createStylesheet(this._styleSheetClass, styleLocation, { nonce: this.opts.nonce });
	...
	_updateContainerHeight() {
		...
		if (row) this.el.style[parent ? "minHeight" : "height"] = row * cellHeight + unit;
		if (parent && !parent.grid.engine.batchMode && Utils.shouldSizeToContent(parent)) parent.grid.resizeToContentCBCheck(parent.el);
		return this;
	}
	/** @internal */
	...
	_writePosAttr(el, n) {
		if (n.x !== void 0 && n.x !== null) el.setAttribute("gs-x", String(n.x));
		if (n.y !== void 0 && n.y !== null) el.setAttribute("gs-y", String(n.y));
	...
	_writeAttr(el, node) {
		...
		for (const key in attrs) if (node[key]) el.setAttribute(attrs[key], String(node[key]));
		else el.removeAttribute(attrs[key]);
		return this;
	}
	...
	*/
	onResize() {
		...
		} else columnChanged = this.checkDynamicColumn();
		if (this._isAutoCellHeight) this.cellHeight();
		this.engine.nodes.forEach((n) => {
			if (n.subGrid) n.subGrid.onResize();
		});
		if (!this._skipInitialResize) this.resizeToContentCheck(columnChanged);
	...
	resizeToContentCheck(delay = false, n = void 0) {
		...
		if (delay && this.hasAnimationCSS()) return setTimeout(() => this.resizeToContentCheck(false, n), 310);
		if (n) {
	...
	_initMargin() {
		...
		let margin = 0;
		let margins = [];
		if (typeof this.opts.margin === "string") margins = this.opts.margin.split(" ");
		if (margins.length === 2) {
		...
		if (this.opts.marginTop === void 0) this.opts.marginTop = margin;
		else {
			data = Utils.parseHeight(this.opts.marginTop);
			this.opts.marginTop = data.h;
			delete this.opts.margin;
		}
		if (this.opts.marginBottom === void 0) this.opts.marginBottom = margin;
		else {
			data = Utils.parseHeight(this.opts.marginBottom);
			this.opts.marginBottom = data.h;
			delete this.opts.margin;
		}
		if (this.opts.marginRight === void 0) this.opts.marginRight = margin;
		else {
			data = Utils.parseHeight(this.opts.marginRight);
			this.opts.marginRight = data.h;
			delete this.opts.margin;
		}
		if (this.opts.marginLeft === void 0) this.opts.marginLeft = margin;
		else {
		...
		this.opts.marginUnit = data.unit;
		if (this.opts.marginTop === this.opts.marginBottom && this.opts.marginLeft === this.opts.marginRight && this.opts.marginTop === this.opts.marginRight) this.opts.margin = this.opts.marginTop;
		return this;
	}
	...
	*/
	static setupDragIn(dragIn, dragInOptions, root = document) {
		if (dragInOptions?.pause !== void 0) DDManager.pauseDrag = dragInOptions.pause;
		dragInOptions = {
	...
	*/
	...
	_removeDD(el) {
		...
		if (el.gridstackNode) delete el.gridstackNode._initDD;
		delete el.ddElement;
		return this;
	}
	...
	_setupAcceptWidget() {
		...
		let onDrag = (event, el, helper) => {
			...
			} else this._dragOrResize(helper, event, ui, node, cellWidth, cellHeight);
		};
		dd.droppable(this.el, { accept: (el) => {
			let node = el.gridstackNode || this._readAttr(el, false);
			if (node?.grid === this) return true;
			if (!this.opts.acceptWidgets) return false;
			let canAccept = true;
			if (typeof this.opts.acceptWidgets === "function") canAccept = this.opts.acceptWidgets(el);
			else {
			...
			let node = el.gridstackNode;
			if (node?.grid === this && !node._temporaryRemoved) return false;
			if (node?.grid && node.grid !== this && !node._temporaryRemoved) node.grid._leave(el, helper);
			cellWidth = this.cellWidth();
			cellHeight = this.getCellHeight(true);
			if (!node) node = this._readAttr(el, false);
			if (!node.grid) {
			...
			if (!node.grid || node.grid === this) {
				...
				if (this._isTemp) this.removeAsSubGrid(node);
			}
			return false;
			...
			if (wasAdded && origNode?.grid && origNode.grid !== this) {
				...
				if (oGrid.parentGridItem && !oGrid.engine.nodes.length && oGrid.opts.subGridDynamic) oGrid.removeAsSubGrid();
			}
			if (!node) return false;
			if (wasAdded) {
			...
			if (helper !== el) {
				...
				if (wasAdded) el = el.cloneNode(true);
			} else {
				el.remove();
				this._removeDD(el);
			}
			...
			el.gridstackNode = node;
			node.el = el;
			let subGrid = node.subGrid?.el?.gridstack;
			...
			if (subGrid) {
				...
				if (!subGrid.opts.styleInHead) subGrid._updateStyles(true);
			}
			this._updateContainerHeight();
			...
			this._triggerChangeEvent();
			this.engine.endUpdate();
			if (this._gsEventHandler["dropped"]) this._gsEventHandler["dropped"]({
				...event,
				type: "dropped"
			}, origNode && origNode.grid ? origNode : void 0, node);
	...
	_setupRemoveDrop() {
		...
		if (!this.opts.staticGrid && !dd.isDroppable(trashEl)) dd.droppable(trashEl, this.opts.removableOptions).on(trashEl, "dropover", (event, el) => GridStack._itemRemoving(el, true)).on(trashEl, "dropout", (event, el) => GridStack._itemRemoving(el, false));
		return this;
	}
	...
	_prepareDragDropByNode(node) {
		...
		if (!node._initDD) {
			...
			let onStartMoving = (event, ui) => {
				if (this._gsEventHandler[event.type]) this._gsEventHandler[event.type](event, event.target);
				cellWidth = this.cellWidth();
				cellHeight = this.getCellHeight(true);
			...
			let onEndMoving = (event) => {
				...
				if (node._isAboutToRemove) {
					...
					if (grid._gsEventHandler[event.type]) grid._gsEventHandler[event.type](event, target);
					...
					} else this._writePosAttr(target, node);
					if (this._gsEventHandler[event.type]) this._gsEventHandler[event.type](event, target);
				}
				this._extraDragRow = 0;
				this._updateContainerHeight();
				this._triggerChangeEvent();
				...
				if (event.type === "resizestop") {
					if (Number.isInteger(node.sizeToContent)) node.sizeToContent = node.h;
	...
	_onStartMoving(el, event, ui, node, cellWidth, cellHeight) {
		...
		if (node.grid?.el) this.dragTransform = Utils.getValuesFromTransformedElement(el);
	...
	_dragOrResize(el, event, ui, node, cellWidth, cellHeight) {
		...
		let mHeight = Math.round(cellHeight * .1), mWidth = Math.round(cellWidth * .1);
		...
		if (event.type === "drag") {
			...
			node._prevYPix = ui.position.top;
			if (this.opts.draggable.scroll !== false) Utils.updateScrollPosition(el, ui.position, distance);
			...
			if (this.engine.collide(node, p)) {
				...
				if (this.opts.maxRow && row + extra > this.opts.maxRow) extra = Math.max(0, this.opts.maxRow - row);
	...
	*/
	_leave(el, helper) {
		...
		if (this.opts.removable === true) GridStack._itemRemoving(el, true);
...
//#endregion
export { DDGridStack, GridStack, GridStackEngine, Utils, dragInDefaultOptions, gridDefaults, obsolete, obsoleteAttr, obsoleteOpts, obsoleteOptsDel };
